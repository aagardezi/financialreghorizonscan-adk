...
FCA_DATASTORE_ID = f"projects/{PROJECT_ID}/locations/global/collections/default_collection/dataStores/{os.environ.get('FCA_VERTEX_SEARCH_INDEX')}"
PRA_DATASTORE_ID = f"projects/{PROJECT_ID}/locations/global/collections/default_collection/dataStores/{os.environ.get('PRA_VERTEX_SEARCH_INDEX')}"
...
class SearchEscalationChecker(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        evaluation_result = ctx.session.state.get("search_research_evaluation")
...
class FCAEscalationChecker(BaseAgent):
    """Checks research evaluation and escalates to stop the loop if grade is 'pass'."""
    ...
    async def _run_async_impl(
        ...
        evaluation_result = ctx.session.state.get("fca_research_evaluation")
        if evaluation_result and evaluation_result.get("grade") == "pass":
            logging.info(
                f"[{self.name}] Research evaluation passed. Escalating to stop loop."
            )
...
class PRAEscalationChecker(BaseAgent):
    """Checks research evaluation and escalates to stop the loop if grade is 'pass'."""
    ...
    async def _run_async_impl(
        ...
        evaluation_result = ctx.session.state.get("pra_research_evaluation")
        if evaluation_result and evaluation_result.get("grade") == "pass":
            logging.info(
                f"[{self.name}] Research evaluation passed. Escalating to stop loop."
            )
...
plan_generator = LlmAgent(
    ...
    **TOOL USE IS STRICTLY LIMITED:**
    ...
    When creating the plan keep in mind this end goal is to use the web search, FCA handbook index and PRA Handbook index as well.
    """,
    tools=[google_search],
)
...
#     Do not include a "References" or "Sources" section in your outline. Citations will be handled in-line.
#     """,
#     output_key="fca_report_sections",
# )

search_section_researcher = LlmAgent(
    model=config.worker_model,
    name="search_section_researcher",
    ...
    **Final Output:** Your final output will comprise the complete set of processed summaries from `[RESEARCH]` tasks AND all the generated artifacts from `[DELIVERABLE]` tasks, presented clearly and distinctly.
    """,
    tools=[google_search],
    output_key="search_section_research_findings",
    after_agent_callback=collect_research_sources_callback,
)

fca_section_researcher = LlmAgent(
    model=config.worker_model,
    name="fca_section_researcher",
    description="Performs the crucial first pass of FCA research.",
    planner=BuiltInPlanner(
        thinking_config=genai_types.ThinkingConfig(include_thoughts=True)
    ),
    instruction="""
    ...
    *   For each `[RESEARCH]` goal:
        *   **Query Generation:** Formulate a comprehensive set of 4-5 targeted search queries. These queries must be expertly designed to broadly cover the specific intent of the `[RESEARCH]` goal from multiple angles.
        *   **Execution:** Utilize the `VertexAiSearchTool` tool to execute **all** generated queries for the current `[RESEARCH]` goal.
        *   **Summarization:** Synthesize the search results into a detailed, coherent summary that directly addresses the objective of the `[RESEARCH]` goal.
        *   **Internal Storage:** Store this summary, clearly tagged or indexed by its corresponding `[RESEARCH]` goal, for later and exclusive use in Phase 2. You **MUST NOT** lose or discard any generated summaries.
    ...
    **Final Output:** Your final output will comprise the complete set of processed summaries from `[RESEARCH]` tasks AND all the generated artifacts from `[DELIVERABLE]` tasks, presented clearly and distinctly.
    """,
    tools=[VertexAiSearchTool(data_store_id=FCA_DATASTORE_ID)],
    output_key="fca_section_research_findings",
    after_agent_callback=collect_research_sources_callback,
)

pra_section_researcher = LlmAgent(
    model=config.worker_model,
    name="pra_section_researcher",
    description="Performs the crucial first pass of PRA research.",
    planner=BuiltInPlanner(
        thinking_config=genai_types.ThinkingConfig(include_thoughts=True)
    ),
    instruction="""
    ...
    **Final Output:** Your final output will comprise the complete set of processed summaries from `[RESEARCH]` tasks AND all the generated artifacts from `[DELIVERABLE]` tasks, presented clearly and distinctly.
    """,
    tools=[VertexAiSearchTool(data_store_id=PRA_DATASTORE_ID)],
    output_key="pra_section_research_findings",
    after_agent_callback=collect_research_sources_callback,
)

search_research_evaluator = LlmAgent(
    model=config.critic_model,
    name="search_research_evaluator",
    description="Critically evaluates research and generates follow-up queries.",
    instruction=f"""
    You are a meticulous quality assurance analyst evaluating the research findings in 'search_section_research_findings'.
    ...
    **CRITICAL RULES:**
    ...
    output_key="search_research_evaluation",
)

fca_research_evaluator = LlmAgent(
    model=config.critic_model,
    name="fca_research_evaluator",
    description="Critically evaluates research and generates follow-up queries.",
    instruction=f"""
    You are a meticulous quality assurance analyst evaluating the research findings in 'fca_section_research_findings'.
    ...
    **CRITICAL RULES:**
    ...
    write a detailed comment about what's missing, and generate 5-7 specific follow-up queries to fill those gaps.
    If the research thoroughly covers the topic, grade "pass".
    If after 5 refinement runs the agent confirms it doenst have the data or the tools to run the refinement, mark it as "pass"
    ...
    disallow_transfer_to_parent=True,
    disallow_transfer_to_peers=True,
    output_key="fca_research_evaluation",
)

pra_research_evaluator = LlmAgent(
    model=config.critic_model,
    name="pra_research_evaluator",
    description="Critically evaluates research and generates follow-up queries.",
    instruction=f"""
    You are a meticulous quality assurance analyst evaluating the research findings in 'pra_section_research_findings'.
    ...
    **CRITICAL RULES:**
    ...
    disallow_transfer_to_parent=True,
    disallow_transfer_to_peers=True,
    output_key="pra_research_evaluation",
)

search_enhanced_search_executor = LlmAgent(
    model=config.worker_model,
    name="search_enhanced_search_executor",
    ...
    1.  Review the 'search_research_evaluation' state key to understand the feedback and required fixes.
    2.  Execute EVERY query listed in 'follow_up_queries' using the 'google_search' tool.
    3.  Synthesize the new findings and COMBINE them with the existing information in 'search_section_research_findings'.
...
fca_enhanced_search_executor = LlmAgent(
    model=config.worker_model,
    name="fca_enhanced_search_executor",
    description="Executes follow-up searches and integrates new findings.",
    planner=BuiltInPlanner(
        thinking_config=genai_types.ThinkingConfig(include_thoughts=True)
    ),
    ...
    1.  Review the 'fca_research_evaluation' state key to understand the feedback and required fixes.
    2.  Execute EVERY query listed in 'follow_up_queries' using the 'VertexAiSearchTool' tool.
    3.  Synthesize the new findings and COMBINE them with the existing information in 'fca_section_research_findings'.
    4.  Your output MUST be the new, complete, and improved set of research findings.
    """,
    tools=[VertexAiSearchTool(data_store_id=FCA_DATASTORE_ID)],
    output_key="fca_section_research_findings",
...
pra_enhanced_search_executor = LlmAgent(
    model=config.worker_model,
    name="pra_enhanced_search_executor",
    description="Executes follow-up searches and integrates new findings.",
    planner=BuiltInPlanner(
        thinking_config=genai_types.ThinkingConfig(include_thoughts=True)
    ),
    ...
    1.  Review the 'pra_research_evaluation' state key to understand the feedback and required fixes.
    2.  Execute EVERY query listed in 'follow_up_queries' using the 'VertexAiSearchTool' tool.
    3.  Synthesize the new findings and COMBINE them with the existing information in 'pra_section_research_findings'.
    4.  Your output MUST be the new, complete, and improved set of research findings.
    """,
    tools=[VertexAiSearchTool(data_store_id=PRA_DATASTORE_ID)],
    output_key="pra_section_research_findings",
...
report_composer = LlmAgent(
    ...
    *   Research Plan: `{research_plan}`
    WEB
    *   Research Findings: `{search_section_research_findings}`
    FCA INDEX
    *   Research Findings: `{fca_section_research_findings}`
    PRA INDEX
    *   Research Findings: `{pra_section_research_findings}`
    OVERALL
    ...
    **The only correct format is:** `<cite source="src-ID_NUMBER" />`
    ...
    Make sure the combine the results of the WEB and FCA INDEX and PRA INDEX research to create a coherent report based on the report_section
    Highlight which part is from the [WEB] research and which is from the [FCA INDEX] and which is from [PRA INDEX]
    """,
    output_key="final_cited_report",
    after_agent_callback=citation_replacement_callback,
)

search_research_executor = SequentialAgent(
    name="search_research_executor",
    description="Executes a pre-approved research plan against Google Search. It performs iterative research, evaluation, and composes a final, cited report.",
    sub_agents=[
        search_section_researcher,
        LoopAgent(
            name="search_iterative_refinement_loop",
            max_iterations=config.max_search_iterations,
            sub_agents=[
                search_research_evaluator,
                SearchEscalationChecker(name="escalation_checker"),
                search_enhanced_search_executor,
            ],
        ),
    ]
)

fca_research_executor = SequentialAgent(
    name="fca_research_executor",
    description="Executes a pre-approved research plan against the FCA data index. It performs iterative research, evaluation, and composes a final, cited report.",
    sub_agents=[
        fca_section_researcher,
        LoopAgent(
            name="fca_iterative_refinement_loop",
            max_iterations=config.max_search_iterations,
            sub_agents=[
                fca_research_evaluator,
                FCAEscalationChecker(name="escalation_checker"),
                fca_enhanced_search_executor,
            ],
        ),
    ],
)

pra_research_executor = SequentialAgent(
    name="pra_research_executor",
    description="Executes a pre-approved research plan against the PRA data index. It performs iterative research, evaluation, and composes a final, cited report.",
    sub_agents=[
        pra_section_researcher,
        LoopAgent(
            name="pra_iterative_refinement_loop",
            max_iterations=config.max_search_iterations,
            sub_agents=[
                pra_research_evaluator,
                PRAEscalationChecker(name="escalation_checker"),
                pra_enhanced_search_executor,
            ],
        ),
    ],
)
...
research_pipeline = SequentialAgent(
    ...
    sub_agents=[
        ...
        ParallelAgent(
            ...
            sub_agents=[
                search_research_executor,
                fca_research_executor,
                pra_research_executor
            ],
        ),
        report_composer,
    ],
